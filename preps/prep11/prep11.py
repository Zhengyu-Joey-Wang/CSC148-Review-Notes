...
def mergesort3(lst: list) -> list:
    ...
    if len(lst) < 2:
        ...
        sub_size = len(lst) // 3
        lst1 = mergesort3(lst[:sub_size])
        lst2 = mergesort3(lst[sub_size:sub_size + sub_size])
        lst3 = mergesort3(lst[sub_size + sub_size:])
...
def merge3(lst1: list, lst2: list, lst3: list) -> list:
    ...
    while i < len(lst1) and j < len(lst2):
        if lst1[i] <= lst2[j]:
            temp_arr.append(lst1[i])
            i += 1
        else:
            temp_arr.append(lst2[j])
            j += 1
    temp_arr = temp_arr + lst1[i:] + lst2[j:]
    i = 0
    j = 0
    arr = []
    while i < len(temp_arr) and j < len(lst3):
        if temp_arr[i] <= lst3[j]:
            arr.append(temp_arr[i])
            i += 1
        else:
            arr.append(lst3[j])
            j += 1
    return arr + temp_arr[i:] + lst3[j:]
...
def kth_smallest(lst: list, k: int) -> Any:
    ...
    p = lst[0]
    small, lager = _partition(lst[1:], p)

    if len(small) == k:
        return p
    elif len(small) < k:
        return kth_smallest(lager, k - len(small) - 1)
    else:
